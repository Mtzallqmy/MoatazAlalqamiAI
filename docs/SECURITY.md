...
| أدوات مدمرة أو ذات تأثير خارجي | delete/reset/clean وpush/deploy والنشر وتثبيت الحزم تتطلب موافقة صريحة مهما كان وضع التشغيل |
| نقل المحادثة تلقائيًا إلى مزود آخر | fallback بين حدود المزودات fail-closed ويتطلب `ApprovedOnce` قبل إرسال history للمزود التالي |
| تسريب GitHub token أثناء استيراد مشروع خاص | يستخدم `GIT_ASKPASS` وبيئة حساسة؛ التوكن لا يدخل command أو `toString` أو نتائج التنفيذ |
| ZIP/TAR خبيث | staging ذري، حدود source/expanded size والعدد، ورفض traversal والروابط والأجهزة وZIP المشفر |
...
5. نجاح النموذج ادعاء غير موثوق؛ يعتمد التسليم على exit code وstdout/stderr ونتيجة الاختبار وdiff.
